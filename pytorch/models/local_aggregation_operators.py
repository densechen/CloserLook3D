...
class PosPool(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        self.output_conv = config.pospool.output_conv or (
            self.in_channels != self.out_channels)

        self.grouper = MaskedQueryAndGroup(
            radius, nsample, use_xyz=False, ret_grouped_xyz=True, normalize_xyz=True)
        if self.output_conv:
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        if self.position_embedding == 'xyz':
            ...
            aggregation_features = neighborhood_features.view(
                B, C // 3, 3, npoint, self.nsample)
            aggregation_features = position_embedding * \
                aggregation_features  # (B, C//3, 3, npoint, nsample)
            aggregation_features = aggregation_features.view(
                B, C, npoint, self.nsample)  # (B, C, npoint, nsample)
        elif self.position_embedding == 'sin_cos':
            feat_dim = C // 6
            wave_length = 1000
            alpha = 100
            feat_range = torch.arange(feat_dim, dtype=torch.float32).to(
                query_xyz.device)  # (feat_dim, )
            dim_mat = torch.pow(1.0 * wave_length,
                                (1.0 / feat_dim) * feat_range)  # (feat_dim, )
            # (B, 3, npoint, nsample, 1)
            position_mat = torch.unsqueeze(alpha * relative_position, -1)
            # (B, 3, npoint, nsample, feat_dim)
            ...
            # (B, 3, npoint, nsample, 2*feat_dim)
            position_embedding = torch.cat([sin_mat, cos_mat], -1)
            ...
            aggregation_features = neighborhood_features * \
                position_embedding  # (B, C, npoint, nsample)
        else:
            raise NotImplementedError(
                f'Position Embedding {self.position_embedding} not implemented in PosPool')
        ...
        if self.reduction == 'max':
            ...
            raise NotImplementedError(
                f'Reduction {self.reduction} not implemented in PosPool ')
...
class AdaptiveWeight(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        if self.weight_type == 'dp':
            # (B, C//S, npoint, nsample)
            conv_weight = self.mlps(relative_position)
            # (B, C//S, 1, npoint, nsample)
            conv_weight = torch.unsqueeze(conv_weight, 2)
        else:
            raise NotImplementedError(
                f'Weight Type {self.weight_type} not implemented in AdaptiveWeight')
...
class PointWiseMLP(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        if self.num_mlps == 1:
            ...
            self.mlps.add_module('conv0', nn.Sequential(
                nn.Conv2d(self.feature_input_channels,
                          mfdim, kernel_size=1, bias=False),
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        if self.feature_type == 'dp_fi_df':
            # B C N M
            center_features = torch.unsqueeze(
                neighborhood_features[..., 0], -1).repeat([1, 1, 1, self.nsample])
            relative_features = neighborhood_features - center_features
            local_input_features = torch.cat(
                [relative_position, center_features, relative_features], 1)
            aggregation_features = self.mlps(local_input_features)
        else:
            raise NotImplementedError(
                f'Feature Type {self.feature_type} not implemented in PointWiseMLP')
        ...
        if self.reduction == 'max':
            ...
            raise NotImplementedError(
                f'Reduction {self.reduction} not implemented in PointWiseMLP')
        return out_features
...
class PseudoGrid(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        self.register_buffer('K_points', torch.from_numpy(
            K_points_numpy).type(torch.float32))

        self.grouper = MaskedQueryAndGroup(
            radius, nsample, use_xyz=False, ret_grouped_xyz=True, normalize_xyz=False)
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        neighborhood_features, relative_position, neighborhood_mask = self.grouper(query_xyz, support_xyz, query_mask,
                                                                                   support_mask, support_features)
        relative_position = torch.unsqueeze(
            relative_position.permute(0, 2, 3, 1), 3)
        relative_position = relative_position.repeat(
        ...
        if self.KP_influence == 'constant':
            ...
            all_weights = torch.clamp(
                1 - torch.sqrt(sq_distances) / self.extent, min=0.0)
            all_weights = all_weights.permute(0, 1, 3, 2)
        ...
        # Mask padding points
        feature_mask = neighborhood_mask + \
            (1 - query_mask[:, :, None])  # B, N, M
        all_weights *= feature_mask[:, :, None, :]  # B, N, K, M

        if self.convolution_mode != 'sum':
            raise NotImplementedError(
                f"convolution_mode:{self.convolution_mode} not support in PseudoGrid")

        # get features for each kernel point
        all_weights = all_weights.view(-1,
                                       self.num_kernel_points, self.nsample)
        neighborhood_features = neighborhood_features.permute(
            0, 2, 3, 1).contiguous().view(-1, self.nsample, C)
        # # [B*N, K, M],[B*N, M, C] -> [B*N, K, C]
        weighted_features = torch.bmm(all_weights, neighborhood_features)
        kernel_outputs = weighted_features * self.kernel_weights  # [B*N, K, C]
        out_features = torch.sum(kernel_outputs, 1).view(
            B, npoint, C).transpose(1, 2)
...
class InvopointOp(nn.Module):
    """Invopoint Operation Module.
    """
    ...
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        self.radius = radius
        self.nsample = nsample
        self.reduction = config.invopoint.reduction
        self.output_conv = config.invopoint.output_conv or (
            self.in_channels != self.out_channels)
...
class LocalAggregation(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        if config.local_aggregation_type == 'pospool':
            self.local_aggregation_operator = PosPool(
                in_channels, out_channels, radius, nsample, config)
        elif config.local_aggregation_type == 'adaptive_weight':
            self.local_aggregation_operator = AdaptiveWeight(
                in_channels, out_channels, radius, nsample, config)
        elif config.local_aggregation_type == 'pointwisemlp':
            self.local_aggregation_operator = PointWiseMLP(
                in_channels, out_channels, radius, nsample, config)
        elif config.local_aggregation_type == 'pseudo_grid':
            self.local_aggregation_operator = PseudoGrid(
                in_channels, out_channels, radius, nsample, config)
        elif config.local_aggregation_type == 'invopoint':
            self.local_aggregation_operator = InvopointOp(
                in_channels, out_channels, radius, nsample, config)
        else:
            raise NotImplementedError(
                f'LocalAggregation {config.local_aggregation_type} not implemented')
